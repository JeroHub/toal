...
- Assume a 0 centered guassian distribution of inter-pulse-intervals (fixed tag pulse interval).
...
- Speed of sound through water is a constant, unknown value.
...
We've setup this function to first estimate the position using spehrical interpolation.
These preliminatry results will then be used as starting values in the YAPS model to speed up the optimization.

```{r,results=F, message=F, warning=F}
## YAPS-Pelagic
estimates.yaps.p <- yaps.pelagic(toa = toa.real, 
                                 hydrophone.pos = hydro.pos,
                                 c = c, max.iterations = 5000)
```
...
results <- rbind(as.matrix(estimates.si[estimates.si$eq == '+',2:4]),
                 as.matrix(estimates.yaps.p[,1:3]),
...
results$Model <- c(rep('SI+', dim(estimates.si[estimates.si$eq == '+',])[1]),
                   rep('YAPS.p', times = dim(estimates.yaps.p)[1]),
...
message('YAPS estimate c: ', attr(estimates.yaps.p, 'c'),
        ', real c: ', c.real)
```
...
```{r}
ggplot(data = rbind(
  data.frame(Hydrophone = 1, ms = 1000*matrix(estimates.yaps.p$dl_H1, ncol = 1)),
  data.frame(Hydrophone = 2, ms = 1000*matrix(estimates.yaps.p$dl_H2, ncol = 1)),
  data.frame(Hydrophone = 3, ms = 1000*matrix(estimates.yaps.p$dl_H3, ncol = 1)),
  data.frame(Hydrophone = 4, ms = 1000*matrix(estimates.yaps.p$dl_H4, ncol = 1)))) +
  geom_histogram(aes(x = ms), bins = 40) + 
